...
local class_mt = {}
class_mt.__index = function (table, key)
    if key == "__init" then
        return function (...)
            class_mt.__index(...)
        end 
    else
        if key == "__childClases" or key == "__redefined" then return end 
        if table.__redefined and table.__redefined[key] then
            return function (...)
                return table.__childClases[1][key](...)
            end
            ...
            function superreturn (...)
                ...
                return setmetatable({}, {
                        __index = function (_, key)
                            if table.__bases[1].__redefined[key] then
                                return function (...)

                                    pcall(table.__bases[1].__redefined[key], table.__bases[1], ...)
                                end
                            end
                        end
                    })
...
local function build(class, shared_objs, shared)
    ...
    if class.__bases[1] then
    local base = class.__bases[1]
    base.__childClases = (not base.__childClases and {class}) 
                                                or table.insert(base.__childClases, class)  
    end
    ...
    if class.__childClases then
       class.__redefined = {}
        
        local child = class.__childClases[1]
        --print("process child", child.__name, child.__inherited)
        for name, f in pairs(child) do
            if class[name] and type(class[name]) == "function" and (child.__inherited[name] ~= child[name]) then
                --print("remove functions", name, "from entity", class)
                class.__redefined[name] = class[name]
                class[name] = nil
            end
        end
    end
...
local function implements(value, class)
    return classof(value) and value:implements(class) or false
end

local function is_a(value, class)
...
function deepcopy(orig)
    ...
    if orig_type == 'table' then
        ...
        for orig_key, orig_value in next, orig, nil do
             ...
             if(orig_key=="__childClases") then
            copy=orig
            else 
